...
export class ChooseActionState {
        ...
        if (isCurrentPlayerActive) {
            ...
            if (this.args.legendaryCharacter === 3 && !this.args.legendaryCharacterState) {
                (this.args._private?.legendaryCharacter3Colors ?? []).forEach(color => {
                    ...
                    this.bga.statusBar.addActionButton(
                        _('Use 2 ${color} as ${locomotive}').replace('${color}', colorIcon).replace('${locomotive}', locomotiveIcon),
                        () => this.bga.actions.performAction('actUseLegendaryCharacter', { color }),
                    );
                });
     ...
     */
    private setActionBarChooseColor(route: Route, possibleColors: number[], showDistributionPopin: boolean) {
    ...
    private getConsideredRouteColor(route: { color: number }): number {
