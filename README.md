...
This project is an adaptation for BoardGameArena of games Ticket to Ride Map packs edited by Days of Wonder.
...
```
If you use it for another game, replace `tickettoridemaps` mentions on package.json `build:scss` script and on tsconfig.json `files` property.
...
go on tests dir and start execute file, for example `php ./tickettoridemaps.game.test-longest-path.php` / `php ./tickettoridemaps.game.test-destination-completed.php`
...
Can you complete the file modules/maps/india/routes.php with all the routes represented as colored rectangles on img/india/map.webp ?
